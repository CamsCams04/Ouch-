...
const DEMO_ROOM = {
  ...
  code: "AX79L",
  members: [
    { id: "u1", name: "Toi", avatar: "🐯", color: C.orange },
    { id: "u2", name: "Pote1", avatar: "🦊", color: C.teal },
    { id: "u3", name: "Raph", avatar: "🐸", color: "#9b59b6" },
    { id: "u4", name: "Kev", avatar: "🦁", color: "#e74c3c" },
  ],
  stats: {
...
const totalScore = (stats, uid) => {
  if (!stats || !uid) return 0;
  return Object.values(stats[uid] || {}).reduce((a, b) => (Number(a) || 0) + (Number(b) || 0), 0);
};
    ...
    <Screen style={{ justifyContent: "center" }}>
      <div className="fade-up" style={{ textAlign: "center", marginBottom: 12 }}>
        ...
        <p style={{ color: C.muted, marginTop: 10, fontSize: 15 }}>
          Le trash-talk entre potes, enfin organisé.
        </p>
      </div>
      ...
      <div className="fade-up" style={{ animationDelay: ".3s" }}>
        <Btn onClick={() => name.trim() && onLogin({ name: name.trim(), avatar: AVATARS[avatarIdx] })}
          disabled={!name.trim()}>
          Entrer dans l'arène 💥
        </Btn>
      </div>
  ...
  return (
    <Screen>
      ...
      <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 12, animationDelay: ".1s" }}>
        ...
        {!joining ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <Input label="Code du salon" value={code} onChange={setCode}
              placeholder="ex: AX79L" maxLength={6} />
            <div style={{ display: "flex", gap: 10 }}>
              <Btn variant="ghost" onClick={() => setJoining(false)} style={{ flex: 1 }}>Annuler</Btn>
              <Btn variant="teal" onClick={() => code.trim() && onJoin(code.trim().toUpperCase())}
                disabled={!code.trim()} style={{ flex: 2 }}>Rejoindre</Btn>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="fade-up" style={{
        marginTop: "auto", background: C.card, borderRadius: 16,
        padding: "16px 20px", border: `1.5px solid ${C.border}`, animationDelay: ".2s",
      }}>
        <p style={{ fontSize: 13, color: C.muted, marginBottom: 10 }}>
          👁️ Voir la démo avec des potes fictifs
        </p>
...
function PageCreateRoom({ user, onCreated, onBack }) {
  ...
  const create = () => {
    ...
    const room = {
      id: uid(), name: roomName.trim(), code: uid().slice(0, 5),
      members: [{ id: "u_me", name: user.name, avatar: user.avatar, color: C.orange }],
      categories: cats,
      stats: { u_me: Object.fromEntries(cats.map(c => [c, 0])) },
    };
    onCreated(room);
  };
  ...
  return (
    <Screen>
      <button onClick={onBack} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 14, width: "fit-content", display: "flex", alignItems: "center", gap: 6 }}>
        ← Retour
      </button>
      ...
      <div>
        <p style={{ fontSize: 13, color: C.muted, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>Catégories d'insultes</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => toggleCat(c)} style={{
              ...
              background: cats.includes(c) ? C.orange : C.card,
              color: cats.includes(c) ? "#fff" : C.muted,
              border: `1.5px solid ${cats.includes(c) ? C.orange : C.border}`,
              transition: "all .15s",
            }}>{c}</button>
          ))}
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <input
            ...
            maxLength={20}
            style={{
              flex: 1, background: C.card, border: `1.5px solid ${C.border}`, borderRadius: 10,
              padding: "10px 14px", color: C.bone, fontSize: 14, fontFamily: "Barlow", outline: "none",
            }}
          />
          <button onClick={addCat} style={{
            background: C.teal, border: "none", borderRadius: 10, padding: "10px 16px",
...
// 4. WAITING ROOM (share code)
function PageWaiting({ room, user, onStart, onBack }) {
  const [copied, setCopied] = useState(false);
  const copy = () => {
    navigator.clipboard.writeText(room.inviteCode || room.code);
    setCopied(true);
  ...
  return (
    <Screen>
      <button onClick={onBack} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 14, width: "fit-content" }}>
        ← Retour
      </button>
      ...
      {/* Code */}
      <div style={{
        textAlign: "center", background: C.card, borderRadius: 20,
        padding: "28px 24px", border: `2px dashed ${C.orange}`,
      }}>
        <p style={{ fontSize: 13, color: C.muted, marginBottom: 8, fontWeight: 600, letterSpacing: 1 }}>CODE D'INVITATION</p>
        <p style={{ fontFamily: "Bebas Neue", fontSize: 56, color: C.orange, letterSpacing: 8 }}>{room.inviteCode || room.code}</p>
      ...
      {/* Members */}
      <div>
        <p style={{ fontSize: 13, color: C.muted, fontWeight: 600, letterSpacing: 1, marginBottom: 12 }}>
        ...
        {room.members.map(m => (
          <div key={m.id} style={{
            ...
            marginBottom: 8, border: `1.5px solid ${C.border}`,
          }}>
            <span style={{ fontSize: 24 }}>{m.avatar}</span>
            <span style={{ fontWeight: 700 }}>{m.name}</span>
            {m.id === "u_me" || m.name === user.name ? (
              <span style={{ marginLeft: "auto", fontSize: 12, color: C.orange, fontWeight: 600 }}>TOI</span>
            ) : null}
          </div>
        ))}
        <div style={{
          padding: "12px 16px", borderRadius: 12, border: `1.5px dashed ${C.border}`,
...
useEffect(() => {
  ...
  return (
    <Screen style={{ gap: 14 }}>
      ...
      {/* Members grid */}
      <p style={{ fontSize: 11, color: C.muted, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase" }}>
        Touche quelqu'un 👇
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, flex: 1 }}>
        {room.members.map(m => {
          ...
          const isMe = memberId === myId || m.name === user.name;
          const isShaking = shaking === m.id;
          const isPopping = popping === m.id;
          const myFloats = floatLabels.filter(l => l.targetId === m.id);

          return (
            <div key={m.userId || m.id} style={{ position: "relative" }}>
              {/* Float labels */}
              {myFloats.map(l => (
                <div key={l.id} className="float-label" style={{
                  top: "50%", left: "50%", transform: "translate(-50%,-50%)",
                  background: C.orange, color: "#fff", padding: "4px 10px",
                  borderRadius: 8, fontSize: 12, fontWeight: 700, zIndex: 10,
                  whiteSpace: "nowrap",
                }}>{l.text}</div>
              ))}
              ...
              >
                {/* Ripple overlay on hit */}
                {isPopping && (
                  <div style={{
                    position: "absolute", inset: 0, background: `${C.orange}22`,
                ...
                <span style={{
                  fontSize: 42,
                  animation: isPopping ? "pop .4s ease forwards" : "none",
                }}>{m.avatar}</span>

                <span style={{ fontWeight: 700, fontSize: 15, color: isMe ? C.muted : C.bone }}>
                  {m.name}
                </span>

                {isMe && (
                  <span style={{ fontSize: 11, color: C.muted }}>C'est toi</span>
                )}

                {/* Score badge */}
                <div style={{
                  background: isMe ? C.border : (total > 20 ? C.orange : C.teal),
                  borderRadius: 20, padding: "4px 12px", minWidth: 50, textAlign: "center",
                }}>
                  <span style={{
                ...
                {!isMe && room.stats[m.id] && (
                  <div style={{ width: "100%", marginTop: 4 }}>
                    {Object.entries(room.stats[m.id])
                      .filter(([, v]) => v > 0)
                      .sort(([, a], [, b]) => b - a)
                      .slice(0, 2)
                      .map(([cat, val]) => (
...
function PageStats({ room, onBack }) {
  const sorted = [...room.members].sort((a, b) => totalScore(room.stats, b.id) - totalScore(room.stats, a.id));
  const maxScore = Math.max(...room.members.map(m => totalScore(room.stats, m.id)), 1);
  ...
  return (
    <Screen>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {sorted.map((m, i) => {
          const score = totalScore(room.stats, m.id);
          const pct = (score / maxScore) * 100;
          const medal = ["🥇", "🥈", "🥉"][i] || "👤";
          return (
            <div key={m.id} onClick={() => setSelected(m.id)} style={{
              background: selected === m.id ? "#2a2a2a" : C.card,
              borderRadius: 16, padding: "14px 16px",
              border: `1.5px solid ${selected === m.id ? C.orange : C.border}`,
              ...
              <div style={{ height: 8, background: C.border, borderRadius: 4, overflow: "hidden" }}>
                <div style={{
                  ...
                  background: i === 0 ? C.orange : C.teal,
      ...
      {selected && (
        <div className="fade-up" style={{ background: C.card, borderRadius: 20, padding: "20px 18px", border: `1.5px solid ${C.border}` }}>
          <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 14, color: C.bone }}>
            {room.members.find(m => m.id === selected)?.avatar} Détail de {room.members.find(m => m.id === selected)?.name}
          </p>
          {allCats.map(cat => {
            const val = room.stats[selected]?.[cat] || 0;
            const catMax = Math.max(...room.members.map(m => room.stats[m.id]?.[cat] || 0), 1);
            return (
              <div key={cat} style={{ marginBottom: 12 }}>
                ...
                <div style={{ height: 6, background: C.border, borderRadius: 3, overflow: "hidden" }}>
                  <div style={{
                    ...
                    width: `${(val / catMax) * 100}%`, transition: "width .5s ease",
                  }} />
                </div>
              </div>
            );
...
export default function App() {
 ...
 const handleCreated = async (r) => {
  // Crée la room en BDD via l'API REST
...
const handleJoin = (code) => {
  ...
  const handleStart = () => setPage("duel");
  ...
  return (
    <>
      ...
      {page === "waiting" && room && (
        <PageWaiting room={room} user={user} onStart={handleStart} onBack={() => setPage("home")} />
      )}
      {page === "duel" && room && (
        <PageDuel
          room={room} user={user}
      ...
      {page === "stats" && room && (
        <PageStats room={room} onBack={() => setPage("duel")} />
      )}
    </>
  );
}
